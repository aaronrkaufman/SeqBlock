...
names(results)  <<- c("pid", "race", "age", "edu", "gender", "regvote", "vote2", "marry", "interest", "income", "reg", "mot", "pow", "res", "treat", "outcome")
...
#  })
  ...
  dynamicUi <- reactive({
    ...
    if (input$Click.Counter==(nrow(Qlist)+1)){ #is +1 what I want?
      ...
      #drop_acc(dtoken = token) # this line is breaking, but maybe I don't need it
      #print("okay so far")
      
      # next step: generate conjoint-style conditions. Block on them as well. 
      reg = regime[rbinom(1,1,0.5)+1]
      mot = motive[rbinom(1,1,0.5)+1]
      pow = power[rbinom(1,1,0.5)+1]
      res = result[rbinom(1,1,0.5)+1]
      
      results[11:14] <<- c(reg, mot, pow, res) 
      ...
      bdata = seqblock(query = FALSE, object = "sdata.RData", id.vars = "ID", id.vals =last+1,
                       exact.vars=c("pid", "race", "age", "edu", "gender", "regvote", "vote2", "marry", "interest", "income",
                                    "regime", "motive", "power", "result"),
                       exact.vals = c(results["pid"], results["race"], results["age"], results["edu"], results["gender"], results["regvote"],
                                      results["vote2"], results["marry"], results["interest"], results["income"], reg, mot, pow, res), 
    ...
    if (input$Click.Counter>=nrow(Qlist)+2){
      ...
      drop_upload(fn, dest="seqblock/results_3_16", overwrite=FALSE)
      #print(result)
      uniqueID = randstring()
      return(list(
